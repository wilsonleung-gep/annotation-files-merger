...
class FileReader {
    ...
    public function __construct($filename) {
        ...
        $this->buildDataLines($eol, $fileContents);
    }
    ...
    private function buildDataLines($eol, $contents) {
        $fileContents = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $contents);

        $this->dataLines = explode($eol, trim($fileContents));
    }

    private function stripRTFTags($fileContents) {
        $pattern = sprintf('/%s/', join("|", array(
            "(\{[\\\\])(.+?)(\})",
            "([\\\\])(.+?)(\b)",
            "[\\\\]",
            "\}",
